...
- 文字列はダブルクォートで統一する（`singleQuote: false`）
- セミコロンあり
- インデントは 2 スペース
- 末尾カンマあり（ES5 準拠）
- ファイル保存時に Prettier が自動フォーマットする（エディタの format on save を有効にすること）
- 手動フォーマットは `npm run format` で実行する

## アクセシビリティ

- ヘッドレス UI ライブラリとして Radix UI を使用する
- インタラクティブなコンポーネント（メニュー・モーダル等）は Radix UI を使ってアクセシビリティを担保する
- 必要なコンポーネントを使うタイミングで個別にインストールする
  - 例: `npm install @radix-ui/react-dialog`
